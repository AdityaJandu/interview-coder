...
export interface IIpcHandlerDeps {
  ...
  moveWindowUp: () => void
  moveWindowDown: () => void
  // NOTE: these were `| undefined` before, which conflicted with the
  // `| null` types held on `state`. Widened to accept null as well so we
  // don't have to fabricate a fake "undefined" value at the call site.
  transcriptionHelper?: import('./TranscriptionHelper').TranscriptionHelper | null
  conversationManager?: import('./ConversationManager').ConversationManager | null
  answerAssistant?: import('./AnswerAssistant').AnswerAssistant | null
}
...
async function createWindow(): Promise<void> {
  ...
  // Local, non-null alias. TS can't keep narrowing `state.mainWindow`
  // (a mutable object property) across the calls below, so we capture
  // it once here and use this local for the rest of the function.
...
function setWindowDimensions(width: number, height: number): void {
  ...
  if (mainWindow && !mainWindow.isDestroyed()) {
    ...
    // Enforce user requested width limit: approx (1/3 + 1/6) = 1/2 of screen width max
    const maxWidth = Math.floor(workArea.width * 0.5)
    // Capping maximum height dynamically so the GPU tile buffer can never explode to 17000px
    ...
    mainWindow.setBounds({
      x: Math.min(currentX, workArea.width - newWidth),
      y: currentY,
      width: newWidth,
      height: newHeight
    })
